...
export default class extends Vue {
  ...
  // @Prop({ required: true }) private pagetype!: number
  @Prop({ required: true }) private url!: string
  @Prop({ required: true }) private fileTypes!: Array<string>
  ...
  private beforeUploadFile(file: any) {
    ...
    if (this.fileTypes.findIndex(fileType => fileType === extension) === -1) {
      this.$notify({
        title: this.$t('uploadFile.notifyWarning').toString(),
        message: '',
        type: 'warning',
      })
      isTrue = false
  ...
  private handleError(err: any, file: any, fileList: any) {
    this.$notify.error({
      title: this.$t('common.error').toString(),
      message: this.$t('common.addError').toString(),
    })
  }
}
</script>
