            ...
            <el-row class="ht" :gutter="10">
              <el-col class="ht" :span="17">
                <div class="his-module his-small">
                  ...
                  <pagenation-list
                    ref="pageTable"
                    data-url="/system-service/clientSystemParameter/queryPage"
                    http-method="post"
                    v-model="tableData"
                  ...
                  >
                    ...
                    >
                      ...
                      <el-table-column label="IP" prop="ip" width="110px" show-overflow-tooltip></el-table-column>
                      <el-table-column label="参数名称" prop="csm" width="85" show-overflow-tooltip></el-table-column>
                      <el-table-column label="参数值" prop="csz" width="100" show-overflow-tooltip></el-table-column>
                      <el-table-column label="备注" prop="bz" width="80"></el-table-column>
                      <el-table-column label="所属类别" show-overflow-tooltip width="70">
                        <template slot-scope="scope">
                          <div>{{transer(scope.row.sslb)}}</div>
                        </template>
                      </el-table-column>
                      <el-table-column label="是否启用" prop="sfqy" width="70">
                        <template slot-scope="scope">
                          <span
                            :class="[scope.row.sfqy == '1' ? 'blues' : 'reds']"
                          >{{scope.row.sfqy == '1' ? '是' : '否'}}</span>
                        </template>
                      </el-table-column>
                      <el-table-column
                        label="操作"
              ...
              <el-col class="ht" :span="7">
                ...
                >
                  ...
                  <el-form label-width="67px" :model="ruleForm" ref="ruleForm" :rules="rule">
                    <el-form-item label="IP" prop="ip" :class="[isUpd ? '' : 'is-disabled']">
                      <el-input v-model="ruleForm.ip" placeholder="请输入..." :disabled="!isUpd" />
                    </el-form-item>
                    <el-form-item label="参数名称" prop="csm" :class="[isUpd ? '' : 'is-disabled']">
                      <el-input v-model="ruleForm.csm" placeholder="请输入..." :disabled="!isUpd"></el-input>
                    </el-form-item>
                    ...
                    <el-form-item label="所属类别">
                      <el-select placeholder="请选择" v-model="ruleForm.sslb" multiple collapse-tags>
                        <el-option
                    ...
                    <el-form-item label="是否启用">
                      <!-- <el-select placeholder="请选择" v-model="ruleForm.sfqy">
                      ...
                      <search-select
                        ...
                        kid="FD000005"
                        v-model="ruleForm.sfqy"
                        :backfillValue="ruleForm.sfqy"
                      ></search-select>
                    </el-form-item>
                  </el-form>
                </div>
...
import { delClientSystem, searchClientSystem, saveClientSystem } from '@/api/his/sys/centerMaintenance/medicareInfoManage/clientParameters.ts'
...
export default class extends Vue {
  ...
  private handleRowData(row: any) {
    ...
    searchClientSystem(params).then((res: any) => {
      console.log(res)
  ...
  private handleAdd() {
    ...
    this.ruleForm.bz = ''
    this.ruleForm.sslb = null
    this.ruleForm.sfqy = null
  }
  //表单验证
  private rule: object = {
    ip: [{ required: true, message: '请输入IP', trigger: 'blur' }],
    csm: [{ required: true, message: '请输入参数名称', trigger: 'blur' }],
    csz: [{ required: true, message: '请输入参数值', trigger: 'blur' }]
  }
  ...
  private handleSave() {
    ;(this.$refs.ruleForm as Form).validate((valid: boolean) => {
      if (valid) {
        saveClientSystem(this.ruleForm).then((res: any) => {
          console.log(res)
          if (res.errorCode === 'SUCCESS') {
            if (this.isUpd) {
              this.$notify({
  ...
  private handleDelete(row: any) {
    ...
    delClientSystem(params).then((res: any) => {
      if (res.errorCode === 'SUCCESS') {
        this.getList()
        this.handleAdd()
        this.$notify({
