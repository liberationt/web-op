...
<template>
  <div>
    <div class="his-module his-small" style="padding: 8px 10px">
      ...
      <pagenation-list
        ref="pageTable"
        data-url="/op-service/pubbrxz/queryFyxzList"
        http-method="post"
        v-model="costData"
      ...
      >
        ...
        >
          ...
          <el-table-column label="单位" prop="fydw"></el-table-column>
          <el-table-column label="自费标志" prop="tsbz">
            <template slot-scope="scope">
              <search-select
                table-name="SYS_FLAG_DATA"
                kid="FD000005"
                v-model="scope.row.tsbz"
                :backfillValue="scope.row.tsbz"
                :placeholder="''"
                @change="getTsbz"
              ></search-select>
            </template>
...
import { addCostLimit, getDrugLimit, editCostLimit, delCostLimit } from '@/api/his/sys/centerMaintenance/medicareInfoManage/patientNature.ts'
...
export default class extends Vue {
  ...
  private FYXH: number | null = null // 药品序号
  private TSBZ: any = null // 特殊标志
  private rowData: any = {} // 获取费用限制表格单行数据
    ...
    if (this.BRXZ) {
      ...
      if (this.isAdd) {
        if (this.costData.length > 0) {
          ...
          if (oldRowData.fymc) {
            this.costData = [{ zfbl: 0, tsbz: 0 }, ...this.costData]
          }
        } else {
          this.costData = [{ zfbl: 0, tsbz: 0 }, ...this.costData]
        }
  ...
  // 获取自费标志
  private getTsbz(val: any) {
    this.TSBZ = val
    if (this.isAdd) {
      this.rowData.tsbz = val
      editCostLimit(this.rowData).then((res: any) => {
  ...
  private handleDelete(row: any) {
    if (this.isAdd) {
      if (this.delFlag) {
        ...
        delCostLimit(params).then((res: any) => {
          if (res.errorCode) {
            this.$notify({
              title: '已删除',
              message: '',
  ...
  private handleSave(row: any) {
    if (row.brxz) {
      ...
      editCostLimit(params).then((res: any) => {
        if (res.errorCode === 'SUCCESS') {
          this.$notify({
            title: '已保存',
            message: '',
      ...
      if (params.zfbl >= 0) {
        addCostLimit(params).then((res: any) => {
          if (res.errorCode) {
            this.$notify({
              title: '已保存',
              message: '',
  ...
  private getDrugTableData(val: any, index: number) {
    console.log(val)
    this.FYXH = val.fyxh
    this.$set(this.costData, index, { ...val, fydw: val.fydw, zfbl: val.zfbl, tsbz: val.tsbz })
  }
