    ...
    >
      ...
      <div slot="header" class="row" v-if="isShow">
        <div class="label">
          ...
          <div class="btn">
            <el-button size="mini" type="text" :disabled="!isAdd || (this.tableDatas.length>0 && this.searchData.sslb == 4)" @click="handleAdd" >
      ...
      >
        ...
        <!-- 组套名称 -->
        <el-table-column prop="groupCode" :label="$t('examineSuite.suiteName')" min-width="100">
        ...
        <!-- 组套类别 -->
        <el-table-column prop="ztlb" :label="$t('examineSuite.suiteType')" min-width="80">
          <template slot-scope="scope">
        ...
        <el-table-column prop="sfqy" :label="$t('common.status')" min-width="60">
          <template slot-scope="scope">
            <span :class="scope.row.sfqy == 1 ? 'blues' : 'reds'">{{ scope.row.sfqy | filterStatus }}</span>
          </template>
        </el-table-column>
        <!-- 操作 -->
        <el-table-column :label="$t('table.actions')" width="75" align="center" v-if="isShow">
          <template slot-scope="scope">
            <!-- 修改,保存按钮 -->
