<template>
  <div class="content ht">
        ...
        <div class="his-module his-small ht">
          <div class="row">
            <div class="label">
              <div class="querySearch">
                <div class="searchTitle no-warp" style="padding-right: 2px;">留观登记单</div>
              </div>
              <div class="btn">
                ...
                <el-button size="mini" type="text" @click="handlePrint(tableData[0].djid)" :disabled="!(this.SQZT==2)"> <i class="icon his-btn-print"></i>打印 </el-button>
                <el-button size="mini" type="text" @click="handleSubmit" :disabled="this.SQZT==2 || this.tableData.length==0"> <i class="icon his-btn-print"></i>提交并打印</el-button>
                <el-button size="mini" type="text" @click="handleCancle(SQZT)" :disabled="!(this.SQZT==2)"> <i class="icon his-btn-print"></i>取消提交 </el-button>
              </div>
            </div>
          </div>
          <pagenation-list 
            ref="pageTable" 
            v-model="tableData" 
            data-url="/op-service/mzzydj/getHospitalizedRegisterList" 
          ...
          >
            ...
            >
              ...
              <!-- 开单日期 -->
              <el-table-column :label="'申请时间'" prop="kdrq" width="90">
              ...
              <!-- 联系人 -->
              <el-table-column :label="'联系人'" width="60" prop="lxrm"></el-table-column>
              <!-- 联系地址 -->
              <el-table-column :label="'联系人地址'" width="70" :show-overflow-tooltip="true" prop="lxdz"></el-table-column>
              <!-- 联系电话 -->
              ...
              <!-- 留观科室 -->
              <el-table-column :label="'留观科室'" width="90" prop="nszksName"></el-table-column>
              <!-- 拟收治病区 -->
              <el-table-column :label="'拟住院病区'" width="90" prop="nszbqName"></el-table-column>
              <!-- 留观住院目的 -->
              <el-table-column :label="'留观住院目的'" width="100" prop="rymdName"></el-table-column>
              <!-- 特需 -->
      ...
      <el-col class="ht" :span="8" style="padding-right: 0px;">
        <div class="his-module his-small">
          ...
          <div class="normal-form-wrap scroll-bar">
            <el-form ref="formData" :model="formData" :rules="formRule" label-width="86px" class="clearfix" style="width: 100%">
              <!-- 联系人 -->
              ...
              <el-form-item :label="'联系地址'" prop="lxdz">
                <el-input :placeholder="'请输入联系地址'" v-model="formData.lxdz"></el-input>
              </el-form-item>

              <!-- 联系电话 -->
              <el-form-item :label="'联系电话'" prop="lxdh">
                <el-input :placeholder="'请输入联系电话'" v-model.number="formData.lxdh"></el-input>
              </el-form-item>
              ...
              <!-- 留观科室 -->
              <el-form-item :label="'留观科室'" prop="nszks">
                <el-select class="w100" v-model="formData.nszks" :placeholder="$t('请选择科室')">
                  <el-option v-for="item in wordList" :key="item.dv" :label="item.dn" :value="item.dv"></el-option>
                </el-select>
              </el-form-item>

              <!-- 拟住院病区-->
              <el-form-item label="拟留观病区" prop="nszbq">
              ...
              <!-- 留观住院目的 -->
              <el-form-item :label="'留观目的'" prop="rymd">
    ...
    <div id="rysqd" style="width: 210mm;margin: 0 auto" v-show="false">
      ...
      <div style="">
        <div style="font-size: 24px;text-align: center;margin:5px 0 15px;">{{ryzPrintData.jgmc}}留观证</div>
        ...
        <table border="1" cellpadding="0" style="border-collapse:collapse;border: 1px solid #000;width: 100%;font-size: 14px">
          ...
          <tr>
            <td style="height: 35px; border: 1px solid #000;line-height: 35px;text-align: center">拟留观病区</td>
            <td style="height: 35px; border: 1px solid #000;line-height: 35px;text-align: center" colspan="2">{{ryzPrintData.nrybq}}</td>
            <td style="height: 35px; border: 1px solid #000;line-height: 35px;text-align: center">拟留观科别</td>
...
import { add, edit, deleteById, updateStatus, queryLogo, printModel, queryPatientInfo } from '@/api/cis/op/dctwork/registerationForm'
...
export default class extends Vue {
  ...
  private kdksList: Array<any> = [] // 开单科室
  private kdysList: Array<any> = [] // 开单医生
  private loGo: any = '' // 医院logo
  private SQZT: any = '' // 申请状态
  private DJID: number = 0 // 登记id
  private formRule: any = {
    // lxdz: [{ required: true, message: '请输入联系地址', trigger: 'blur' }],
    lxdh: [
      { required: true, message: '请输入联系电话', trigger: 'blur' },
      { pattern: regPhoneTel, message: window.ele.$i18n.t('emp.vdPhone'), trigger: 'blur' },
    ],
    nszbq: [{ required: true, message: '请选择病区', trigger: 'blur' }],
    nszks: [{ required: true, message: '请选择科室', trigger: 'blur' }],
  } // 表单验证
  private relationshipList: Array<any> = [] // 与患者关系数组
  private aimList: Array<any> = [] // 入院目的
  ...
  get tableFormat() {
    ...
    id2Name(tabledata, 'nszbq', 'nszbqName', this.inpatientList, 'dv', 'dn')
    id2Name(tabledata, 'nszks', 'nszksName', this.wordList, 'dv', 'dn')
  ...
  mounted() {
    ...
    this.wordList = Vue.prototype.$qiankun ? Vue.prototype.$store.webSqlDict.OBSERVE_DEPARTMENT_DICT : this.$store.state.webSqlDict.OBSERVE_DEPARTMENT_DICT // 住院科室
    this.kdksList = Vue.prototype.$qiankun ? Vue.prototype.$store.webSqlDict.DEPT_DICT : this.$store.state.webSqlDict.DEPT_DICT
    ...
    this.getLists()
    if (this.patientFile) {
      this.searchData.mzhm = this.patientFile.mzhm
      ...
      this.formData = {
        ...
        mzks: this.patientFile.zsid,
        mzys: this.patientFile.ysdm,
        zzd: this.patientFile.fymc,
        zzdicd10: this.patientFile.icd10,
        lxrm: this.patientFile.lxrm,
        lxdz: this.patientFile.lxdz,
        lxdh: this.patientFile.lxdh,
        lxrgx: this.patientFile.lxgx,
        djlx: '0',
        nszbq: '',
        rymd: '0',
      }
    }
    if(this.patientFile.lxgx != null) {
      this.formData.lxrgx = this.patientFile.lxgx.toString()
    }
  }
  ...
  private handleAdd() {
    ...
    // 获取患者信息
    queryPatientInfo({ jzkh:this.patientFile.jzkh }).then((res: any)=> {
      this.$nextTick(() => {
        this.formData = {
          brid: this.patientFile.brid,
          brxb: this.patientFile.brxb,
      ...
      if (valid) {
        if (this.isAdd) {
          ...
          if (this.formData.sqzt == 2) {
            this.$notify.warning('该单据已提交，请先取消提交后修改')
            return
          } else if (this.formData.sqzt == 3) {
            this.$notify.warning('该单据已到住院状态，不允许修改')
            return
          }
  ...
  private handleSubmit() {
    ...
    updateStatus(param).then(() => {
      ...
      this.getLists()
      this.$nextTick(()=> {
        this.handlePrint(this.tableData[0].djid)
      })
    })
  }
