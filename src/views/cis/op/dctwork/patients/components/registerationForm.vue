      ...
      <el-col class="ht" :span="16">
        <div class="his-module his-small ht">
          ...
          <pagenation-list ref="pageTable" v-model="tableData" data-url="/op-service/mzzydj/getHospitalizedRegisterList" :search-data="searchData" http-method="post" :list-height="listSlotHeight">
            <el-table slot="list" stripe ref="table" :highlight-current-row="true" :data="tableFormat" @row-click="handleEdit" :row-class-name="tableRowClassName" border v-loading="loading" :height="listSlotHeight" class="scroll-bar" tooltip-effect="light">
              <!-- 序号 -->
          ...
          <div class="normal-form-wrap scroll-bar">
            <el-form ref="formData" :model="formData" :rules="formRule" label-width="86px" class="clearfix">
              ...
              <el-form-item :label="'联系人'" prop="lxrm">
                <el-input ref="lxrm" :placeholder="'请输入联系人'" v-model="formData.lxrm" @blur="checkRm" clearable></el-input>
              </el-form-item>
              ...
              <el-form-item :label="'联系人地址'" prop="lxdz">
                <el-input :placeholder="'请输入联系人地址'" v-model="formData.lxdz" clearable></el-input>
              </el-form-item>

              <!-- 联系电话 -->
              <el-form-item :label="'联系人电话'" prop="lxdh">
                <el-input ref="lxdh" :placeholder="'请输入联系人电话'" v-model="formData.lxdh" @blur="checkDh" clearable></el-input>
              </el-form-item>
              ...
              <!-- 住院主要目的 -->
              <el-form-item :label="'住院主要目的'" prop="rymd">
              ...
              <el-form-item label="预缴费用" prop="yjfy">
                <el-input v-model="formData.yjfy" placeholder="请输入" type="number" clearable></el-input>
              </el-form-item>
...
import { add, edit, deleteById, updateStatus, printModel, queryLogo, queryPatientInfo } from '@/api/cis/op/dctwork/registerationForm'
...
export default class extends Vue {
  ...
  private BRXM: any = '' // 本人姓名
  private BRDH: any = '' // 本人电话
  //数据交互
  ...
  private checkRm() {
    debugger
    if(this.formData.lxrm == this.BRXM) { 
      this.formData.lxrm = this.patientFile.lxrm
      this.$message.warning('联系人姓名与本人姓名重复')
      this.$nextTick(()=> {
        ;(this.$refs['lxrm'] as any).focus()
      })
    }
  }
  ...
  // 联系人电话重复判断
  private checkDh() {
    if(this.formData.lxdh == this.BRDH) { 
      this.formData.lxdh = this.patientFile.lxdh
      this.$message.warning('联系人电话与本人电话重复')
      this.$nextTick(()=> {
        ;(this.$refs['lxdh'] as any).focus()
      })
    }
  }
  ...
  mounted() {
    ...
    if(this.userInfo.hospitalId !== 310112001){
      let lxdz = { required: true, message: '请输入联系地址', trigger: 'blur' }
      let lxrm = { required: true, message: '请输入联系人姓名', trigger: 'blur' }
      let lxrgx = { required: true, message: '请输入联系人关系', trigger: 'blur' }
      this.formRule.lxdz.push(lxdz)
      this.formRule.lxrm.push(lxrm)
      this.formRule.lxrgx.push(lxrgx)
    }
    this.getLists()
    this.handleAdd()
  }
  ...
  // 新增按钮
  private handleAdd() {
    console.log('646',this.patientFile)
    ...
    queryPatientInfo({ jzkh:this.patientFile.jzkh }).then((res: any)=> {
      this.$nextTick(() => {
        ...
        this.BRXM = res.data.brxm
        this.BRDH = res.data.brdh
        this.txbz = false
        this.dbjfbz = false
      })
    })
  ...
  private delDept(row: any) {
    if (row.sqzt == 2) {
      this.$notify.warning('请先取消提交后删除')
      return
    } else if (row.sqzt == 3) {
      this.$notify.warning('已入院不允许删除')
      return
    }
