...
//==获取LODOP对象主过程,判断是否安装、需否升级:== src\plugins\lodop\CLodop_Setup.exe
export function getLodop(oOBJECT, oEMBED) {
    ...
    var strCLodopInstall_1 = "<br><font color='#FF00FF'>Web打印服务CLodop未安装启动，点击这里→<a href='./download/CLodop_Setup.exe' target='_self'>下载执行安装</a>";
    ...
    try {
        ...
        if (needCLodop()) {
            ...
            if (!LODOP) {
                ...
                MessageBox.confirm('Web打印服务CLodop未安装或未启动，是否下载？', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    console.log('从子项目下载')
                    location.href='./download/CLodop_Setup.exe'
                }).catch(() => {
                    console.log('从子项目取消下载')     
                });
